...
class Command(BCommand):
    ...
    def process(self, *args, **kwargs):
        ...
        if forced:
            items = ConsumptionQue.objects.prefetch_related('robotpos').all()
        else:
            items = ConsumptionQue.waiting.prefetch_related('robotpos').all()
        ...
        for item in items:
            ...
            if forced is False and ConsumptionQueLog.objects.filter(
            ...
            try:
                result = ConsumptionQue.generate_fiche(
                    item.identifier,
                    item.robotpos,
                    item.sales,
                    item
                ...
                logger.exception(e)
                ConsumptionQueLog.objects.create(
                    content_type=ContentType.objects.get_for_model(item),
                    object_id=item.id,
                    exception=str(e)
                )
