...
class PurchaseOrderQue(Que, CommonQue):
    @classmethod
    def validate(cls, data):
        request = Active.settings['DEFAULT_PURCHASE_ORDER'].copy()
        request.update(data)
        po = PurchaseOrder(
            data=request
        )
    ...
    def generate_fiche(cls, data):
        ...
        po = PurchaseOrder(
            data=remove_empty_from_dict(request)
        )
        po.is_valid(raise_exception=True)
        po.transfer_via_rest()
        return po


class PurchaseOrderQueLog(QueLog):
    pass


class PurchaseDispatchQue(Que, CommonQue):
    @classmethod
    def validate(cls, data):
        request = Active.settings['DEFAULT_PURCHASE_DISPATCH'].copy()
        request.update(data)
        pd = PurchaseDispatch(
            data=request
        )
    ...
    def generate_fiche(cls, data):
        ...
        pd.is_valid(raise_exception=True)
        pd.transfer_via_rest()

        check = all([
            pd.flow,
            pd.flow.success,
            pd.flow.internal_ref
        ])

        if check:
            refund = []

            if 'data' in odata and 'TRANSACTIONS' in odata.get('data'):
                for item in odata.get('data').get('TRANSACTIONS').get('items'):
                    quantity = int(item.get('QUANTITY'), 0)
                    procamount = int(item.get('PROCAMOUNT'), 0)
                    if quantity - procamount > 0:
                        item.update({'QUANTITY': quantity - procamount})
                        refund.append(item)

            if refund:
                rf_request = Active.settings['DEFAULT_PURCHASE_DISPATCH'].copy()
                rf_request.update(odata.get('data'))
                rf_request.update({
                    'TYPE': 6,
                    'TRANSACTIONS': {
                        'items': refund
                ...
                rf_pd = PurchaseDispatch(
                    data=remove_empty_from_dict(rf_request),
                    prev_flow=pd.flow
                )
                rf_pd.is_valid(raise_exception=True)
                rf_pd.transfer_via_rest()

        return pd


class PurchaseDispatchQueLog(QueLog):
    pass


class ExpanseQue(Que, CommonQue):
    @classmethod
    def validate(cls, data):
        request = Active.settings['DEFAULT_EXPANSE'].copy()
        request.update(data)
        pi = PurchaseInvoice(
            data=request
        )
        pi.is_valid(raise_exception=True)
    ...
    def generate_fiche(cls, data):
        ...
        pi.is_valid(raise_exception=True)
        pi.transfer_via_rest()
        return pi


class ExpanseQueLog(QueLog):
    pass


class RefundQue(Que, CommonQue):
    @classmethod
    def validate(cls, data):
        request = Active.settings['DEFAULT_REFUND'].copy()
        request.update(data)
        mf = MaterialFiche(
            data=request
        )
