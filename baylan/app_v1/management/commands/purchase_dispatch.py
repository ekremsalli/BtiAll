...
class Command(BCommand):
    ...
    def process(self, *args, **kwargs):
        ...
        if forced:
            orders = PurchaseDispatchQue.objects.all()
        else:
            orders = PurchaseDispatchQue.waiting.all()
        ...
        for item in orders:
            ...
            if forced is False and PurchaseDispatchQueLog.objects.filter(
            ...
            try:
                result = PurchaseDispatchQue.generate_fiche(
                    item.get_data()
                )
            except Exception as e:
                logger.exception(e)
                PurchaseDispatchQueLog.objects.create(
                    content_type=ContentType.objects.get_for_model(item),
                    object_id=item.id,
                    exception=str(e)
                )
