...
class Command(BCommand):
    ...
    def process(self, *args, **kwargs):
        ...
        if forced:
            orders = PurchaseOrderQue.objects.all()
        else:
            orders = PurchaseOrderQue.waiting.all()
        ...
        for item in orders:
            ...
            if forced is False and PurchaseOrderQueLog.objects.filter(
            ...
            try:
                result = PurchaseOrderQue.generate_fiche(
                    item.get_data()
                )
            except Exception as e:
                logger.exception(e)
                PurchaseOrderQueLog.objects.create(
                    content_type=ContentType.objects.get_for_model(item),
                    object_id=item.id,
                    exception=str(e)
                )
