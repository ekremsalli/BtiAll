...
class Command(BCommand):
    ...
    def process(self, *args, **kwargs):
        ...
        if forced:
            refunds = RefundQue.objects.all()
        else:
            refunds = RefundQue.waiting.all()

        if ids:
            refunds = refunds.filter(id__in=ids)

        for item in refunds:

            if forced is False and RefundQueLog.objects.filter(
            ...
            try:
                result = RefundQue.generate_fiche(
                    item.get_data()
                )
            except Exception as e:
                logger.exception(e)
                RefundQueLog.objects.create(
                    content_type=ContentType.objects.get_for_model(item),
                    object_id=item.id,
                    exception=str(e)
                )
