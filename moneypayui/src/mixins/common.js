...
export const common = {
  ...
  filters: {
    ...
    pretty_xml: function(val){
      let xmlFormatter = require('xml-formatter')
      return xmlFormatter(JSON.parse(val))
    }
  },
  methods: {
    ...
    perm_check: function(perm){
      return this.multi_perm_check([perm])
    },
    multi_perm_check: function(perms) {
      if (!this.$store.getters['account/logged']) return false
      if (this.$store.getters['account/is_superuser'] || this.$store.getters['account/is_staff'])  return true
      let current = this.$store.getters['account/perms']
      
      if (Object.keys(current).length == 0) return false
      return perms.some(o => current.includes(o))
