...
class Command(BCommand):
    ...
    def process(self, *args, **kwargs):
        ...
        if forced:
            items = WasteQue.objects.prefetch_related('robotpos').all()
        else:
            items = WasteQue.waiting.prefetch_related('robotpos').all()
        ...
        for item in items:
            ...
            if forced is False and WasteQueLog.objects.filter(
            ...
            try:
                result = WasteQue.generate_fiche(
                    item.identifier,
                    item.robotpos,
                    item.sales,
                    item
